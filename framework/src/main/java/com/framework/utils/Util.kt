...
fun Double.roundToFloat(numFractionDigits: Int): Float =
    "%.${numFractionDigits}f".format(this, Locale.ENGLISH).toFloat()
...
fun AppCompatTextView.setIconifiedText(
    text: String,
    ...
    SpannableStringBuilder("$text#").apply {
        ...
        if (textFont.isNullOrEmpty().not() && font != null) {
            val style =
                ResourcesCompat.getFont(this@setIconifiedText.context, font)?.style ?: Typeface.BOLD
        ...
        if (iconResId != null) {
            ...
            setSpan(
                ImageSpan(context, iconResId, align),
                text.length,
                text.length + 1,
...
fun makeSectionOfTextBold(
    text: String,
    textToBold: String,
    @ColorRes color: Int = android.R.color.black,
    @FontRes font: Int = R.font.semi_bold
    ...
    if (textToBold.isNotEmpty() && textToBold.trim { it <= ' ' } != "") {
        ...
        if (startingIndex < 0 || endingIndex < 0) {
            ...
            builder.append(text)
            val font =
                ResourcesCompat.getFont(BaseApplication.instance, font)?.style ?: Typeface.BOLD
...
suspend fun Bitmap.shareAsImage(packageName: String? = null, text: String? = null) {
        ...
        try {
            ...
            uri = FileProvider.getUriForFile(
                ...
                "${BaseApplication.instance.packageName}.provider",
                file
            )
        ...
        try {
            ...
            if (showNoti) {
                if (progress >= 100) {
                    ...
                    NotiUtils.notificationManager?.notify(
            ...
            Toast.makeText(BaseApplication.instance, "Failed To Save Image", Toast.LENGTH_SHORT)
                .show()
            NotiUtils.notificationManager?.cancel(noti_id)
    ...
    fun getAppVersionName(): String? {
        try {
            val pInfo: PackageInfo = BaseApplication.instance.packageManager.getPackageInfo(
    ...
    inline fun <reified T> convertJsonToObj(json: String?) =
