...
fun Context.glideLoad(mImageView: CustomImageView?, resourceId: Int?) {
  if (mImageView == null) return
  Glide.with(this).load(resourceId).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.DATA).into(mImageView)
}
