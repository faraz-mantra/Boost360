...
private fun SimpleExoPlayer.fullScreenExoPlayer(
  context: AppCompatActivity,
...
private fun SimpleExoPlayer.fullScreenCloseExoplayer(
  context: AppCompatActivity,
  forceLandscape: Boolean,
  normalScreenButton: ImageView,
  playerView: PlayerView,
  playerViewFullscreen: PlayerView
) {
...
fun SimpleExoPlayer.setSource(playbackPosition: Long, context: Context, url: String) {
  val dataSourceFactory: DataSource.Factory =
    DefaultDataSourceFactory(context, Util.getUserAgent(context, "Boost"))
  val videoSource: MediaSource =
    if (url.endsWith("m3u8") || url.endsWith("m3u")) HlsMediaSource.Factory(dataSourceFactory)
