...
class SpringDotsIndicator @JvmOverloads constructor(
  ...
  private fun buildDot(stroke: Boolean): ViewGroup {
    ...
    val dotView = dot.findViewById<ImageView>(R.id.dot)
    dotView.setBackgroundResource(if (stroke) R.drawable.dot_stroke_back else R.drawable.dot_back)
  ...
  private fun setUpDotBackground(stroke: Boolean, dotView: View) {
    val dotBackground = dotView.findViewById<View>(R.id.dot).background as GradientDrawable
    if (stroke) {
