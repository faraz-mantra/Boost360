...
class FlipView : ViewFlipper, SVGPictureDrawable, View.OnClickListener {
  ...
  override var pictureDrawable: PictureDrawable? = null
    set(drawable) {
      ...
      if (frontImageView == null) {
        Log.w(
          TAG,
          "ImageView not found in the first child of the FrontLayout. Image cannot be set!"
        )
        return
      }
  ...
  private fun init(attrs: AttributeSet?) {
    ...
    val animateDesignChildViewsOnly =
      a.getBoolean(R.styleable.FlipView_animateDesignLayoutOnly, false)
    if (!animateDesignChildViewsOnly) {
      ...
      if (frontBackground == null) setChildBackgroundColor(
        FRONT_VIEW_INDEX,
      ...
      if (rearBackground == null) setChildBackgroundColor(
        REAR_VIEW_INDEX,
        rearBackgroundColor
    ...
    mainAnimationDuration =
      a.getInteger(R.styleable.FlipView_animationDuration, FLIP_DURATION).toLong()
    rearImageAnimationDuration =
      a.getInteger(R.styleable.FlipView_rearImageAnimationDuration, REAR_IMAGE_ANIMATION_DURATION)
        .toLong()
    rearImageAnimationDelay =
      a.getInteger(R.styleable.FlipView_rearImageAnimationDelay, mainAnimationDuration.toInt())
        .toLong()
    anticipateInAnimationTime =
      a.getInteger(R.styleable.FlipView_anticipateInAnimationTime, 0).toLong()
    if (!isInEditMode) {
      //This also initialize the in/out animations
      setMainAnimationDuration(mainAnimationDuration)
      if (a.getBoolean(
          R.styleable.FlipView_animateRearImage,
          true
        )
    ...
    initialLayoutAnimationDuration =
      a.getInteger(R.styleable.FlipView_initialLayoutAnimationDuration, INITIAL_ANIMATION_DURATION)
        .toLong()
  ...
  override fun setEnabled(enabled: Boolean) {
    ...
    if (isAutoStart) {
      if (!enabled) stopFlipping() else postDelayed(
        { if (isEnabled) startFlipping() },
        mFlipInterval.toLong()
      )
    }
  }
 ...
 */
  ...
  private fun initInAnimation(@IntRange(from = 0) duration: Long) {
    ...
    super.getInAnimation().startOffset =
      if (anticipateInAnimationTime > duration) duration else duration - anticipateInAnimationTime
  }
  ...
  fun setInitialLayoutAnimation(@AnimRes animationResId: Int) {
    try {
      setInitialLayoutAnimation(
        if (animationResId > 0) AnimationUtils.loadAnimation(
          context,
          animationResId
        ) else createScaleAnimation()
      ) //Usage of the method it's faster (not read from disk)
      if (DEBUG) Log.d(TAG, "Initial animation is active!")
    } catch (e: Resources.NotFoundException) {
      Log.e(
        TAG, "Initial animation with id " + animationResId
  ...
  fun setInitialLayoutAnimation(initialLayoutAnimation: Animation) {
    ...
    if (initialLayoutAnimation.interpolator == null) initialLayoutAnimation.interpolator =
  ...
  fun setRearImageAnimation(@AnimRes animationResId: Int) {
    try {
      ...
      Log.e(
        TAG,
        "Rear animation with id $animationResId could not be found. Rear animation cannot be set!"
      )
    }
  }
  ...
  private fun animateRearImageIfNeeded() {
    if (isFlipped && rearImageView != null && rearImageAnimation != null) {
      rearImageView!!.alpha =
        0f //Alpha 0 and Handler are needed to avoid the glitch of the rear image
      Handler().postDelayed({
  ...
  fun addRearLayout(view: View) {
    ...
    if (view is ViewGroup) {
      ...
      viewGroup = view
      whichChild =
        0 //Override the index: use the first position to locate the ImageView in this ViewGroup
    }
    ...
    if (viewGroup.getChildAt(whichChild) is ImageView) {
      ...
      rearImageView =
        null //Rollback in case multiple views are added (user must provide already the image in each layout added)
    }
  ...
  fun setChildBackgroundDrawable(whichChild: Int, @DrawableRes drawableResId: Int) {
    try {
      setChildBackgroundDrawable(
        whichChild,
        if (hasLollipop()) context.getDrawable(drawableResId) else context.resources.getDrawable(
          drawableResId
        )
      )
    } catch (e: Resources.NotFoundException) {
  ...
  companion object {
     ...
     */
    @JvmOverloads
    fun resetLayoutAnimationDelay(
      enable: Boolean = true,
      @IntRange(from = 0) nextDelay: Long = DEFAULT_INITIAL_DELAY.toLong()
    ) {
    ...
    fun stopLayoutAnimation() {
      Handler().postDelayed(
        { resetLayoutAnimationDelay(false, DEFAULT_INITIAL_DELAY.toLong()) },
        STOP_LAYOUT_ANIMATION_DELAY.toLong()
      )
    }
    //*******************
    // SHAPE DRAWABLES **
    ...
    fun createArcShapeDrawable(
      @ColorInt color: Int, startAngle: Float, sweepAngle: Float
    ): ShapeDrawable {
    ...
    fun createRoundRectShapeDrawable(
      @ColorInt color: Int, outerRadii: FloatArray?, inset: RectF?, innerRadii: FloatArray?
    ): ShapeDrawable {
