...
class ImagePicker(builder: Builder) {
  ...
  class Builder(context: Activity) : ImagePickerBuilderBase {
    ...
    override fun directory(directory: Directory?): Builder {
      when (directory) {
        Directory.DEFAULT -> imageConfig.directory =
          BaseApplication.instance.applicationContext.getExternalFilesDir(Environment.DIRECTORY_PICTURES)?.absolutePath + ImageTags.Tags.IMAGE_PICKER_DIR
        else -> {
        }
      }
      return this
