...
class UserSessionManager(var activity: Context) {
  ...
  /*
    ...
    private val KEY_MERCHANT_USER_NAME = "merchantUserName"
    private val KEY_MERCHANT_USER_EMAIL = "merchantUserEmail"
    private val KEY_MERCHANT_USER_MOBILE = "merchantUserMobile"
    private val KEY_MERCHANT_IS_EMAIL_VERIFIED = "merchantIsEmailVerfied"*/
    ...
    val fPParentId: String?
        get() {
            ...
            if (TextUtils.isEmpty(id)) id =
                pref.getString(UserSessionManager.Companion.KEY_FP_ID, null)
            return id
        }
        ...
        if (cursor.count > 0) {
            if (cursor.moveToNext()) {
                ...
                if (LoginStatus == "true") {
                    ...
                    if (facebookAccessToken != null && facebookAccessToken.trim { it <= ' ' }
                            .isNotEmpty()) {
    ...
    //Merchant User Profile Attributes Prefs
    var merchantUserProfileData: String?
        get() = pref.getString(KEY_MERCHANT_USER_DATA, null)
        set(merchantUserProfile) {
          editor.putString(KEY_MERCHANT_USER_DATA, merchantUserProfile)
          editor.commit()
        }

    var isMerchantUserDetailsSaved: Boolean
    ...
    /*var merchantUserImageUrl: String?
        get() = pref.getString(KEY_MERCHANT_USER_IMAGE_URL, null)
        set(imageUrl) {
            editor.putString(KEY_MERCHANT_USER_IMAGE_URL, imageUrl)
            editor.commit()
        }

    var merchantUserName: String?
        get() = pref.getString(KEY_MERCHANT_USER_NAME, null)
        set(userName) {
            editor.putString(KEY_MERCHANT_USER_NAME, userName)
            editor.commit()
        }

    var merchantUserEmail: String?
        get() = pref.getString(KEY_MERCHANT_USER_EMAIL, null)
        set(email) {
            editor.putString(KEY_MERCHANT_USER_EMAIL, email)
            editor.commit()
        }

    var merchantUserMobile: String?
        get() = pref.getString(KEY_MERCHANT_USER_MOBILE, null)
        set(mobile) {
            editor.putString(KEY_MERCHANT_USER_MOBILE, mobile)
            editor.commit()
        }

    var merchantIsEmailVerified: Boolean?
        get() = pref.getBoolean(KEY_MERCHANT_IS_EMAIL_VERIFIED, false)
        set(isEmailVerified) {
            editor.putBoolean(KEY_MERCHANT_IS_EMAIL_VERIFIED, isEmailVerified!!)
            editor.commit()
        }*/
