        ...
        .addInterceptor(new Interceptor() {
          ...
          public Response intercept(@NonNull Chain chain) throws IOException {
            ...
            if (chain.request().headers().get("Authorization") == null) {
              String token = "";
              if (url.contains("withfloats.com") || url.contains("nowfloats.com")) {
                token = getAuthToken();
              }
              Request request = original.newBuilder().header("Authorization", token).method(original.method(), original.body()).build();
