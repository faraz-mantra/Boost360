...
//            processOnClickOfFabButton()
    ...
    private boolean loadDataFromDb(int skip, boolean isNewMessage) {
        ...
        if (listMessage != null && !listMessage.isEmpty())
            onBusinessUpdateAddedOrUpdated(listMessage.size());
        return true;
...
//    final Intent shareIntent = new Intent(Intent.ACTION_SEND);
        ...
        switch (type) {
            ...
            case "facebook":
                ...
                break;
            default:
                ContentSharing.Companion.shareUpdates(requireActivity(), message, url, rootAliasURI + "/" + subDomain, session.getUserPrimaryMobile(), false, false, imageUri);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == STORAGE_CODE) {
            if (grantResults != null && grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(current_Activity, "Permission denied to read your External storage", Toast.LENGTH_SHORT).show();
        }
    }
