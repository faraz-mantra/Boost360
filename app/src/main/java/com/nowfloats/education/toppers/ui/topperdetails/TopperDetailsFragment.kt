...
class TopperDetailsFragment(private val topperData: Data?, private val isEditing: Boolean) :
  BaseFragment(), RuntimePermissionListener {
    ...
    if (!addUpdateTopper) {
      if (validate() && imageValidate()) {
        ...
        if (viewModel.getTestimonialImagePath() != null) {
          viewModel.getToppersImagesUrl(
            viewModel.getProfileImagePath()!!,
            viewModel.getTestimonialImagePath()!!
          )
        } else {
    ...
    viewModel.apply {
      addTopperResponse.observe(viewLifecycleOwner, Observer {
        ...
        if (!it.isNullOrBlank()) {
          Toast.makeText(
            requireContext(),
            getString(R.string.topper_added_successfully),
            Toast.LENGTH_SHORT
          ).show()
      ...
      updateTopperResponse.observe(viewLifecycleOwner, Observer {
        ...
        if (!it.isNullOrBlank()) {
          Toast.makeText(
            requireContext(),
            getString(R.string.topper_updated_successfully),
            Toast.LENGTH_SHORT
          ).show()
          (activity as ToppersActivity).popFragmentFromBackStack()
        }
      ...
      uploadImageResponse.observe(viewLifecycleOwner, Observer {
        ...
        if (addUpdateTopper) {
          ...
          viewModel.updateOurTopper(
            binding.toppersData as Data,
            profileImageUrl,
            testimonialImageUrl
          )
        } else {
      ...
      rightButton.setOnClickListener {
        when {
          addUpdateTopper -> {
            ...
            Toast.makeText(
              requireContext(),
              getString(R.string.no_candidate_data_found),
              Toast.LENGTH_SHORT
            ).show()
          }
        }
  ...
  private fun imageValidate(): Boolean {
    if (viewModel.getProfileImagePath() == null) {
      Toast.makeText(
        requireContext(),
        getString(R.string.please_select_topper_image),
        Toast.LENGTH_SHORT
      ).show()
      return false
    }
  ...
  companion object {
    ...
    fun newInstance(topperData: Data, isEditing: Boolean): TopperDetailsFragment =
      TopperDetailsFragment(topperData, isEditing)
  }
}