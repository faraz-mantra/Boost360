...
class ProcessApiSyncModel(
  ...
  fun getDataStart(channels: ArrayList<ChannelModel>?): ArrayList<ProcessApiSyncModel> {
    ...
    val selectedItems = channels?.map {
      it.recyclerViewType = RecyclerViewItemType.API_PROCESS_CHANNEL_ITEM.getLayout(); it
    }
    list.add(ProcessApiSyncModel("Setting up your new business website...", null, businessProfile))
    list.add(
      ProcessApiSyncModel(
        "Connecting your business to " + getChannelTxt(channels?.size),
        selectedItems,
        null
      )
    )
  ...
  fun getDataSuccess(channels: ArrayList<ChannelModel>?): ArrayList<ProcessApiSyncModel> {
    ...
    list.add(
      ProcessApiSyncModel(
        "Business connected to " + getChannelTxt(channels?.size),
        selectedItems,
        null,
        status = SyncStatus.SUCCESS.name
      )
    ...
    list.add(
      ProcessApiSyncModel(
        "Your free digital plan activated",
        null,
  ...
  fun getDataErrorFP(channels: ArrayList<ChannelModel>?): ArrayList<ProcessApiSyncModel> {
    ...
    list.add(
      ProcessApiSyncModel(
        "Error in creating business website",
        null,
        businessProfile,
    ...
    list.add(
      ProcessApiSyncModel(
        "Error in connecting to " + getChannelTxt(channels?.size),
        selectedItems,
        null,
        status = SyncStatus.ERROR.name
      )
    ...
    list.add(
      ProcessApiSyncModel(
        "Error in activating your plan",
        null,
        "•     Activating plan features...",
        status = SyncStatus.ERROR.name
      )
    )
    return list
  ...
  fun getDataErrorChannels(channels: ArrayList<ChannelModel>?): ArrayList<ProcessApiSyncModel> {
    ...
    list.add(
      ProcessApiSyncModel(
        "Error in connecting to " + getChannelTxt(count),
        selectedItems,
        null,
        status = SyncStatus.ERROR.name
      )
  ...
  fun getDataErrorActivatePlan(channels: ArrayList<ChannelModel>?): ArrayList<ProcessApiSyncModel> {
    ...
    list.add(
      ProcessApiSyncModel(
        "Business connected to " + getChannelTxt(count),
        selectedItems,
        null,
        status = SyncStatus.SUCCESS.name
      )
