...
class OneActionBuilder(
  buttonText: String, private val autoDismiss: Boolean, private val fadDuration: Long = 0L,
  private val onClick: OnClick?, private val onLongClick: OnClick?,
  @ColorRes private val colorRes: Int?, @DrawableRes private val drwableId: Int?
) : ContentBuilder() {
  ...
  override fun init(view: View) {
    ...
    (actionButton.parent as ViewGroup).fadeOut(fadDuration).doOnComplete {
      ...
      if (drwableId == null) {
        if (colorRes != null) (actionButton.parent as ViewGroup).setBackgroundColor(
      ...
      } else (actionButton.parent as ViewGroup).background =
        ContextCompat.getDrawable(appC, drwableId)
