...
fun BottomDialogBuilder.oneButton(text: String, @ColorRes colorId: Int? = null,
                                  @DrawableRes drwableId: Int? = null, autoDismiss: Boolean = true,
                                  fadDuration: Long = 0L, listener: (ClickListenerSetter.() -> Unit)? = null): BottomDialogBuilder {
    val cbSetter = ClickListenerSetter()
    listener?.invoke(cbSetter)
    footerBuilder = OneActionBuilder(text, autoDismiss, fadDuration, cbSetter._onClick, cbSetter._onLongClick, colorId, drwableId)
    return this
}
...
fun BottomDialogBuilder.featureMutableList(data: SectionsFeature, autoDismiss: Boolean = true): BottomDialogBuilder {
