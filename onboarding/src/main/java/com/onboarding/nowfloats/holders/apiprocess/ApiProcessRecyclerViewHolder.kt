...
class ApiProcessRecyclerViewHolder(binding: ItemApiCallingProcessBinding) : AppBaseRecyclerViewHolder<ItemApiCallingProcessBinding>(binding), RecyclerItemClickListener {
    ...
    private fun setChannelAdapter(list: ArrayList<ChannelModel>?) {
        list?.let { data ->
            activity?.let {
                ...
                if (model?.status == ProcessApiSyncModel.SyncStatus.PROCESSING.name) channelAdapter?.runLayoutAnimation(binding.channelRecycler)
            }
        }
    }
