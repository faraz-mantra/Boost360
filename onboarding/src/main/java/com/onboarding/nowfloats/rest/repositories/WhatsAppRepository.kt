...
object WhatsAppRepository : AppBaseRepository<WebActionRemoteDataSource, AppBaseLocalService>() {
  ...
  fun getWhatsappBusiness(request: String?): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.getWhatsAppBusiness(request = request), Taskcode.GET_BUSINESS_WHATSAPP)
  }
