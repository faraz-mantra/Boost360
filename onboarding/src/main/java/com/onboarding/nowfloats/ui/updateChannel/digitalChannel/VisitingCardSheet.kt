...
open class VisitingCardSheet :
    ...
    override fun onCreateView() {
        showSimmer(true)
        viewModel?.getMerchantProfile(localSessionModel?.floatingPoint)
            ?.observeOnce(viewLifecycleOwner, {
                if (it.isSuccess()) {
                    ...
                    localSessionModel?.contactName = when {
                        localSessionModel?.contactName.isNullOrEmpty()
                            .not() -> localSessionModel?.contactName
                        userDetail?.userName.isNullOrEmpty().not() -> userDetail?.userName
                        else -> localSessionModel?.fpTag
                    }
                    localSessionModel?.primaryNumber =
                        if (localSessionModel?.primaryNumber.isNullOrEmpty()
                                .not()
                        ) localSessionModel?.primaryNumber else if (userDetail?.userMobile.isNullOrEmpty()
                                .not()
                        ) userDetail?.userMobile else ""
                    localSessionModel?.primaryEmail =
                        if (localSessionModel?.primaryEmail.isNullOrEmpty()
                                .not()
                        ) localSessionModel?.primaryEmail else if (userDetail?.userEmail.isNullOrEmpty()
                                .not()
                        ) userDetail?.userEmail else ""

                    val userProfile = ProfileProperties(
                        userName = localSessionModel?.contactName,
                        userMobile = localSessionModel?.primaryNumber,
        ...
        binding?.backBtn?.setOnClickListener {
            ...
            popup.setOnMenuItemClickListener { menu ->
                if (menu.itemId == R.id.menu_edit_contact) {
                    ...
                    WebEngageController.trackEvent(
                        MY_BUSINESS_CARD_MENU_DIGITAL_CHANNEL,
                        CLICK,
                        TO_BE_ADDED
                    )
...
//        dismiss()
    ...
    private fun setAdapterCard(cardList: ArrayList<DigitalCardData>) {
        ...
        binding?.pagerDigitalCard?.apply {
            val adapterPager3 =
                AppBaseRecyclerViewAdapter(baseActivity, cardList, this@VisitingCardSheet)
    ...
    private fun shareCardWhatsApp(messageCard: String?, isWhatsApp: Boolean) {
        ...
        try {
            val cropBitmap =
                bitmap?.let { Bitmap.createBitmap(it, 40, 0, bitmap.width - 80, bitmap.height) }
            ...
            baseActivity.startActivity(
                Intent.createChooser(
                    waIntent,
                    resources.getString(R.string.share_your_business_card)
                )
            )
            dismiss()
    ...
    private fun getBundle(): Bundle {
        ...
        bundle.putString(
            ...
            localSessionModel?.experienceCode
        )
