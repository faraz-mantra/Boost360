...
class ChannelConfirmDialogAnimator {
  ...
  private fun largeLogoSizeAnimators(
    imageRiyaParams: ConstraintLayout.LayoutParams, imageRiyaLargeParams: FrameLayout.LayoutParams
  ): Pair<ValueAnimator, ValueAnimator> {
    val heightAnimator = ValueAnimator.ofInt(
      (imageRiyaLarge?.get()?.measuredHeight?.minus(20))
        ?: 0,
...
//            imageRiyaLarge?.get()?.radius = imageRiyaLargeParams.height.div(2).toFloat()
    ...
    val widthAnimator = ValueAnimator.ofInt(
      (imageRiyaLarge?.get()?.measuredWidth?.minus(20))
        ?: 0,
