  ...
  override fun onItemClick(position: Int, item: BaseRecyclerViewItem?, actionType: Int) {
    ...
    when (actionType) {
      RecyclerViewActionType.CHANNEL_DISCONNECT_CLICKED.ordinal -> {
        ...
        if (count > 0) {
          if (count == 1) binding?.syncBtn?.text = "${activity!!.resources.getString(R.string.continue_syncing)} $count ${activity!!.resources.getString(R.string.string_channel)}"
          else binding?.syncBtn?.text = "${activity!!.resources.getString(R.string.continue_syncing)} $count ${activity!!.resources.getString(R.string.string_channels)}"
