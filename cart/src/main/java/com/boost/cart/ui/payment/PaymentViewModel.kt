...
import com.boost.cart.data.api_model.PaymentThroughEmail.PaymentThroughEmailRequestBody
      ...
      CompositeDisposable().add(
        ApiService.getStates(auth,clientId)
          ...
          .subscribe(
            ...
            {
              val temp = (it as HttpException).response()!!.errorBody()!!.string()
              val errorBody : Error = Gson().fromJson(temp,object : TypeToken<com.boost.cart.data.api_model.stateCode.Error>() {}.type)
              progressBar.visibility = View.GONE
