...
data class BusinessSetupHighData(
  ...
  fun getData(score: Int, visitor: String, order: String, orderText: String = "Bookings", enquiry: String): ArrayList<BusinessSetupHighData> {
    ...
    list.add(BusinessSetupHighData(title1 = "Business\nSnapshot", siteVisitor = Specification("Site visitors", visitor, BusinessClickEvent.WEBSITE_VISITOR.name), booking = Specification("$orderText", order, BusinessClickEvent.ODER_APT.name), enquiry = Specification("Enquiries", enquiry, BusinessClickEvent.ENQUIRIES.name), type = ActiveViewType.IS_BUSINESS_UPDATE.name))
