...
class GrowthStatsData(
  ...
  fun getData(
    ...
    list.add(
      GrowthStatsData(
        title = "Unique visits",
        value = summary?.getNoOfUniqueViews(),
        icon1 = R.drawable.ic_uniqe_visit_d,
        type = GrowthType.UNIQUE_VISITS.name
      )
    )
    list.add(
      GrowthStatsData(
        title = "Address views",
        value = mapVisitCount,
        icon1 = R.drawable.ic_address_view_d,
        type = GrowthType.ADDRESS_NEWS.name
      )
    )
    list.add(
      GrowthStatsData(
