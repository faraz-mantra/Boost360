...
object DevBoostKitRepository : AppBaseRepository<DevBoostKitRemoteData, AppBaseLocalService>() {
  ...
  fun getUpgradePremiumBanner(auth: String = DEVELOPER_ID): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.getUpgradePremiumBanner(auth=auth), TaskCode.GET_UPGRADE_PREMIUM_BANNER)
  }
  ...
  fun getSearchAnalytics(website_id: String?, startDate: String?, endDate: String?, auth: String = DEVELOPER_ID): Observable<BaseResponse> {
    return makeRemoteRequest(
      remoteDataSource.getSearchAnalytics(auth, website_id, startDate, endDate), TaskCode.GET_SEARCH_ANALYTICS
    )
  }
