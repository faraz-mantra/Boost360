...
//    binding?.drawerView?.imgBusinessLogo, binding?.drawerView?.backgroundImage, binding?.drawerView?.txtDomainName
    ...
    private fun registerFirebaseToken() {
        viewModel.getFirebaseToken().observe(this, {
            ...
            token?.let { it1 ->
                FirebaseAuth.getInstance().signInWithCustomToken(it1)
    ...
    private fun intentDataCheckAndDeepLink(intent: Intent?) {
        ...
        if (intent?.extras != null) {
            ...
            if (intent.extras!!.containsKey("payload")) mPayload =
                ...
                if (mDeepLinkUrl != null && mDeepLinkUrl.toString().contains("onelink", true)) {
                    ...
                    if (!checkIsHomeDeepLink()) {
                        val deepHashMap: HashMap<DynamicLinkParams, String> =
                            DynamicLinksManager().getURILinkParams(url)
    ...
    private fun isAppFlyerLink() {
        if (!checkIsHomeDeepLink()) {
            if (AppsFlyerUtils.sAttributionData.containsKey(DynamicLinkParams.viewType.name)) {
                ...
                val buyItemKey =
                    AppsFlyerUtils.sAttributionData[DynamicLinkParams.buyItemKey.name] ?: ""
    ...
    fun setPercentageData(score: Int) {
        ...
        (binding?.drawerView?.progressBar?.layoutParams as? ConstraintLayout.LayoutParams)?.matchConstraintPercentWidth =
            percentage
    ...
    private fun setUserData() {
        ...
        binding?.cartCountTxt?.text =
            "$cartCount ${if (cartCount > 1) "items" else "item"} waiting in cart"
        ...
        data.forEach { it1 ->
            when (it1.navType) {
                DrawerHomeData.NavType.NAV_ORDER_APT_BOOKING.name -> {
                    it1.title =
                        getDefaultTrasactionsTaxonomyFromServiceCode(session?.fP_AppExperienceCode)
                }
...
//      if (mPaths.isNullOrEmpty().not()) uploadSecondaryImage(mPaths[0])
    ...
    override fun onClick(v: View?) {
        ...
        when (v) {
            ...
            binding?.viewCartCount -> session?.let {
                this.initiateAddonMarketplace(
                    it,
                    true,
                    "",
                    ""
    ...
    private fun navClickEvent(type: DrawerHomeData.NavType) {
        when (type) {
            ...
            DrawerHomeData.NavType.NAV_MANAGE_CONTENT -> session?.let {
                this.startManageContentActivity(
                    it
                )
            }
            ...
            DrawerHomeData.NavType.NAV_BOOST_KEYBOARD -> session?.let {
            ...
            DrawerHomeData.NavType.NAV_HELP_SUPPORT -> session?.let {
                this.startHelpAndSupportActivity(
                    it
                )
            }
            ...
            if (it.isSuccess()) {
                if (it.stringResponse.isNullOrEmpty().not()) {
                    session?.storeFPDetails(
                        Key_Preferences.GET_FP_DETAILS_BG_IMAGE,
                        it.stringResponse
                    )
    ...
    private fun getRequestImageDate(businessImage: File): UploadFileBusinessRequest {
        ...
        val fileName =
            takeIf { businessImage.name.isNullOrEmpty().not() }?.let { businessImage.name }
