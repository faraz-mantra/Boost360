...
class DashboardActivity : AppBaseActivity<ActivityDashboardBinding, DashboardViewModel>(), OnItemSelectedListener, RecyclerItemClickListener {
    ...
    override fun onCreateView() {
        ...
        initialize()
        session?.let {
            initData(it.fpTag ?: "", it.fPID ?: "", clientId)
            it.fPID?.let { initLaunchDarkly(fpId = it) }
        }
    }

    private fun initLaunchDarkly(fpId: String) {
        LDManager.init(application, fpId)
    }
            ...
            if (session?.isLoginCheck == true) {
                if (uri != null && uri.toString().contains("onelink", true)) {
                    ...
                    if (deepHashMap.containsKey(DynamicLinkParams.viewType)) {
                        ...
                        if (deepLinkUtil != null) deepLinkUtil?.deepLinkPage(viewType
                                ?: "", buyItemKey ?: "", false)
                    } else deepLinkUtil?.deepLinkPage(data?.substring(data.lastIndexOf("/") + 1)
                            ?: "", "", false)
                }
            } else {
            ...
            if (it.isSuccess()) {
                if (it.stringResponse.isNullOrEmpty().not()) {
                    ...
                    binding?.drawerView?.bgImage?.let { it1 ->
                        glideLoad(it1, it.stringResponse
