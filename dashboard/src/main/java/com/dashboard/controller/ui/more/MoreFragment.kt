...
class MoreFragment : AppBaseFragment<FragmentMoreBinding, DashboardViewModel>(),
    ...
    private fun setData() {
        ...
        if (featureImageUrl.isNullOrEmpty()
                .not() && featureImageUrl!!.contains("BizImages") && featureImageUrl.contains("http")
                .not()
        ) {
        ...
        if (businessLogoUrl.isNullOrEmpty()
                .not() && businessLogoUrl!!.contains("BizImages") && businessLogoUrl.contains("http")
                .not()
        ) {
        ...
        binding?.rivCurrentlyManage?.apply {
            baseActivity.glideLoad(
                this,
                url = bgImageUri,
                placeholder = R.drawable.general_services_background_img_d,
                isCrop = true
            )
        }
    ...
    private fun setRecyclerView() {
        viewModel?.getMoreSettings(baseActivity)?.observeOnce(viewLifecycleOwner, {
            ...
            binding?.rvUsefulLinks?.adapter = AppBaseRecyclerViewAdapter(
                baseActivity,
                data?.usefulLinks as ArrayList<UsefulLinksItem>,
                this
            )
            binding?.rvAbout?.adapter = AppBaseRecyclerViewAdapter(
                baseActivity,
                data.aboutAppSection as ArrayList<AboutAppSectionItem>,
                this
            )
        })
    }
    ...
    override fun onItemClick(position: Int, item: BaseRecyclerViewItem?, actionType: Int) {
        when (actionType) {
            RecyclerViewActionType.ABOUT_VIEW_CLICK.ordinal -> {
                ...
                aboutAppSectionItem?.icon?.let { AboutAppSectionItem.IconType.fromName(it) }
                    ?.let { clickAppActionButton(it) }
            }
            RecyclerViewActionType.USEFUL_LINKS_CLICK.ordinal -> {
                val usefulLinksItem = item as? UsefulLinksItem
                usefulLinksItem?.icon?.let { UsefulLinksItem.IconType.fromName(it) }
                    ?.let { clickUsefulButton(it) }
            }
        }
    }
    ...
    private fun clickUsefulButton(type: UsefulLinksItem.IconType) {
        when (type) {
            ...
            UsefulLinksItem.IconType.ria_digital_assistant -> baseActivity.startHelpAndSupportActivity(
                session!!
            )
    ...
    private fun clickAppActionButton(type: AboutAppSectionItem.IconType) {
        when (type) {
            ...
            frequently_asked_question -> {
                baseActivity.startMobileSite(
                    session,
                    getString(R.string.setting_faq_url),
                    ABOUT_BOOST_FAQS
                )
            }
            terms_of_usages -> {
                baseActivity.startMobileSite(
                    session,
                    resources.getString(R.string.settings_tou_url),
                    ABOUT_BOOST_TNC
                )
            }
            ...
            privacy_policy -> {
                baseActivity.startMobileSite(
                    session,
                    resources.getString(R.string.settings_privacy_url),
                    ABOUT_BOOST_PRIVACY
                )
            }
            like_us_on_facebook -> {
    ...
    private fun trainingCertification() {
        if (session?.getStoreWidgets()?.contains("MERCHANT_TRAINING") == true) {
            baseActivity.startMobileSite(
                session,
                getString(R.string.product_training_link),
                ABOUT_BOOST_TRAINING
            )
        } else {
            WebEngageController.trackEvent(ABOUT_BOOST_TRAINING, CLICK, NO_EVENT_VALUE)
            showDialog(
                baseActivity,
                getString(R.string.restricted_access),
                getString(R.string.you_need_to_buy_the_one_time_pack_for_boost)
    ...
    private fun showPopupWindow(anchor: View) {
        val view =
            LayoutInflater.from(baseActivity).inflate(R.layout.popup_window_logout_menu, null)
    ...
    private fun likeUsFacebook(context: Context, review: String) {
        ...
        try {
            ...
            Toast.makeText(
                context,
                context.getString(R.string.unable_to_open_facebook),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
