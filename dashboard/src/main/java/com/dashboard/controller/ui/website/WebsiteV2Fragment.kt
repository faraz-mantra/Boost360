...
class WebsiteV2Fragment : AppBaseFragment<FragmentWebsiteV2Binding, DashboardViewModel>() {
    ...
    private fun clickActionButton(type: WebsiteActionItem.IconType) {
        when (type) {
            ...
            WebsiteActionItem.IconType.all_images -> startBackgroundActivity(
                session,
            ...
            WebsiteActionItem.IconType.project_teams -> baseActivity.startListProjectAndTeams(
                session
            )
            ...
            WebsiteActionItem.IconType.trip_adviser_ratings -> baseActivity.startListTripAdvisor(
                session
            )
            WebsiteActionItem.IconType.seasonal_offers -> baseActivity.startListSeasonalOffer(
                session
            )
            WebsiteActionItem.IconType.website_theme -> baseActivity.startWebsiteTheme(session)
            WebsiteActionItem.IconType.doctor_e_profile_listing -> baseActivity.startListDoctors(
                session
            )
        }
    }
...
//      binding?.contactDetail -> baseActivity.startBusinessInfoEmail(session)
            ...
            binding?.tvClickHere -> showShortToast(getString(R.string.coming_soon))
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        item?.findViewById<LinearLayoutCompat>(R.id.share_whatsapp)
            ?.setOnClickListener { shareWhatsAppText() }
        ...
        republishWebsiteBtn?.setOnClickListener {
            RepublishProgressBottomSheet().apply {
                ...
                show(
                    ...
                    RepublishProgressBottomSheet::javaClass.name
                )
            }
            this.popupWindow?.dismiss()
        }
    ...
    private fun setUpButtonSelectedUI(isContentSelected: Boolean = true) {
        if (isContentSelected) {
            ...
            binding?.tvClickHere?.text = getString(R.string.content_click_here_text)
        } else {
            ...
            binding?.tvClickHere?.text = getString(R.string.setting_click_here_text)
        }
    }
}
