...
class DashboardFragment : AppBaseFragment<FragmentDashboardBinding, DashboardViewModel>(),
    ...
    private var adapterPagerBusinessUpdate: AppBaseRecyclerViewAdapter<BusinessSetupHighData>? =
        null
                ...
                channels.forEachIndexed { index, channelStatusData ->
                    viewModel?.getChannelsInsight(session?.fPID, channelStatusData.accountType)
                        ?.observeOnce(viewLifecycleOwner, { it2 ->
                            ...
                            if (response2?.isSuccess() == true) channelStatusData.insightsData =
                                response2.data
            ...
            if (channels.isNotEmpty()) {
                ...
                if (adapterSocialMedia == null) {
                    rvMediaChannel.apply {
                        adapterSocialMedia = AppBaseRecyclerViewAdapter(
                            baseActivity,
                            channels,
    ...
    private fun setSummaryAndDrScore(isLoadingShimmerDr: Boolean) {
        (baseActivity as? DashboardActivity)?.setPercentageData(
                ...
                if (isHighDrScore.not()) {
                    ...
                    drScoreSetupList?.map { it1 ->
                        it1.recyclerViewItemType =
                            RecyclerViewItemType.BUSINESS_SETUP_ITEM_VIEW.getLayout()
                    }
                    binding?.pagerBusinessSetupLow?.apply {
//            binding?.motionOne?.transitionToStart()
                        adapterBusinessContent = AppBaseRecyclerViewAdapter(
                            baseActivity,
                            drScoreSetupList!!,
                            this@DashboardFragment
                        )
...
//    binding?.motionOne?.loadLayoutDescription(takeIf { position == 0 }?.let { R.xml.fragment_dashboard_scene } ?: R.xml.fragment_dashboard_scene_hide)
    ...
    private fun apiBusinessSummary() {
        viewModel?.getSellerSummaryV2_5(
            ...
            viewModel?.getUserSummary(clientId, session?.fPParentId, scope)
    ...
    private fun setRoiBusinessReport(
        ...
        roiData.map {
            it.recyclerViewItemType = RecyclerViewItemType.ROI_SUMMARY_ITEM_VIEW.getLayout()
        }
        ...
        if (adapterRoi == null) {
            binding?.rvRoiSummary?.apply {
                adapterRoi =
                    AppBaseRecyclerViewAdapter(baseActivity, roiData, this@DashboardFragment)
                adapter = adapterRoi
            }
    ...
    private fun setWebsiteReport(
        summary: SummaryEntity?,
        mapVisitCount: String,
        countSearch: Int?
    ) {
        ...
        if (adapterGrowth == null) {
            binding?.rvGrowthState?.apply {
                adapterGrowth = AppBaseRecyclerViewAdapter(
                    baseActivity,
                    growthStatsList,
                    this@DashboardFragment
                )
        ...
        binding?.pagerRiaAcademy?.apply {
            if (academyBanner.isNotEmpty()) {
                ...
                if (adapterAcademy == null) {
                    adapterAcademy = AppBaseRecyclerViewAdapter(
                        baseActivity,
                        academyBanner,
                        this@DashboardFragment
                    )
        ...
        binding?.pagerBoostPremium?.apply {
            if (marketBannerFilter.isNotEmpty()) {
                marketBannerFilter.map {
                    it.recyclerViewItemType =
                        RecyclerViewItemType.BOOST_PREMIUM_ITEM_VIEW.getLayout()
                }
                binding?.boostPremiumView?.visible()
                if (adapterMarketBanner == null) {
                    adapterMarketBanner = AppBaseRecyclerViewAdapter(
                        baseActivity,
                        marketBannerFilter,
                        this@DashboardFragment
                    )
...
//        session?.let { baseActivity.startOldSiteMeter(it) }
            ...
            RecyclerViewActionType.QUICK_ACTION_ITEM_CLICK.ordinal -> {
                ...
                QuickActionItem.QuickActionType.from(data.quickActionType)
                    ?.let { quickActionClick(it) }
            }
            RecyclerViewActionType.BUSINESS_ADD_ONS_CLICK.ordinal -> {
                ...
                ManageBusinessData.BusinessType.fromName(data.businessType)?.let {
                    com.dashboard.controller.ui.allAddOns.businessAddOnsClick(
                        it,
                        baseActivity,
                        session
                    )
    ...
    private fun uploadBusinessLogo(businessLogoImage: File) {
        ...
        viewModel?.putUploadBusinessLogo(
            clientId2, fpId = fpId,
            reqType = "sequential",
            reqId = s_uuid, totalChunks = "1", currentChunkNumber = "1",
    ...
    private fun quickActionClick(type: QuickActionItem.QuickActionType) {
        when (type) {
            ...
            QuickActionItem.QuickActionType.ADD_PHOTO_GALLERY -> baseActivity.startAddImageGallery(
                session
            )
            QuickActionItem.QuickActionType.ADD_TESTIMONIAL -> baseActivity.startTestimonial(
                session,
                true
            )
            QuickActionItem.QuickActionType.ADD_CUSTOM_PAGE -> baseActivity.startCustomPage(
                session,
                true
            )
            ...
            QuickActionItem.QuickActionType.POST_SEASONAL_OFFER -> baseActivity.startAddSeasonalOffer(
                session
            )
            QuickActionItem.QuickActionType.LIST_TOPPER -> baseActivity.startListToppers(session)
            QuickActionItem.QuickActionType.ADD_UPCOMING_BATCH -> baseActivity.startListBatches(
                session
            )
            QuickActionItem.QuickActionType.ADD_NEARBY_ATTRACTION -> baseActivity.startNearByView(
                session
            )
            QuickActionItem.QuickActionType.ADD_FACULTY_MEMBER -> baseActivity.startFacultyMember(
                session
            )
    ...
    private fun clickBusinessUpdate(type: BusinessSetupHighData.BusinessClickEvent) {
        when (type) {
            BusinessSetupHighData.BusinessClickEvent.WEBSITE_VISITOR -> baseActivity.startSiteViewAnalytic(
                session,
                "UNIQUE",
                WEBSITE_REPORT_UNIQUE_VISITS
            )
            BusinessSetupHighData.BusinessClickEvent.ENQUIRIES -> baseActivity.startBusinessEnquiry(
                session
            )
            BusinessSetupHighData.BusinessClickEvent.ODER_APT -> baseActivity.startAptOrderSummary(
                session
            )
        }
    }
...
//        baseActivity.startRevenueSummary(session)
    ...
    private fun clickGrowthStats(type: GrowthStatsData.GrowthType) {
        when (type) {
            ...
            GrowthStatsData.GrowthType.ADDRESS_NEWS -> baseActivity.startSiteViewAnalytic(
                session,
                "MAP_VISITS",
                WEBSITE_REPORT_ADDRESS_VISITS
            )
            GrowthStatsData.GrowthType.NEWSLETTER_SUBSCRIPTION -> baseActivity.startSubscriber(
                session
            )
    ...
    private fun actionChannelClick(data: ChannelModel) {
        ...
        if (data.isWhatsAppChannel().not()) {
            if (data.isGoogleSearch()) {
                ...
                title = data.channelAccessToken?.userAccountName?.takeIf { it.isNotEmpty() }
                    ?: data.getName()
        ...
        when {
            data.ctaFileLink.isNullOrEmpty().not() -> {
                ...
                if (ActivityCompat.checkSelfPermission(
                    ...
                    loader.setData(
                        R.raw.download_gif,
                        resources.getString(R.string.download_file_banner)
                    )
    ...
    private fun showSimmerDrScore(isSimmer: Boolean, isRetry: Boolean = false) {
        binding?.mainContent?.post {
            ...
            binding?.retryDrScore?.visibility =
                if (isSimmer.not() && isRetry) View.VISIBLE else View.GONE
            binding?.lowHighViewDrScore?.visibility =
                if (isSimmer && isRetry) View.GONE else View.VISIBLE
        }
    }
    ...
    private fun getWhatsAppData(
        ...
        viewModel?.getWhatsappBusiness(session?.fpTag, WA_KEY)?.observeOnce(this, {
            ...
            if (isBusinessCardShare) businessWebsiteDetailMessage(
                urlStringN,
                isBusinessCardShare = true
            )
            else if (isEnquiriesShare) businessWebsiteDetailMessage(
                urlStringN,
                shareType = shareType
            )
        })
