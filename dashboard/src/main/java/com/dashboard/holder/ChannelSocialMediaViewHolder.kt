...
class ChannelSocialMediaViewHolder(binding: ItemSocialMediaBinding) :
  AppBaseRecyclerViewHolder<ItemSocialMediaBinding>(binding) {
  ...
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    ...
    data?.getCardColor()
      ?.let { getColor(it)?.let { it1 -> binding.cardView.setCardBackgroundColor(it1) } }
