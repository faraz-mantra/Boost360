...
class DateFilterViewHolder(binding: ItemFilterDateBinding) :
  AppBaseRecyclerViewHolder<ItemFilterDateBinding>(binding) {

  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    super.bind(position, item)
    val data = item as? FilterDateModel ?: return
    getColor(if (data.isSelect) R.color.lightest_grey else R.color.white)?.let {
    ...
    binding.mainView.setOnClickListener {
      listener?.onItemClick(
        position,
        item,
        RecyclerViewActionType.DATE_FILTER_CLICK.ordinal
      )
    }
