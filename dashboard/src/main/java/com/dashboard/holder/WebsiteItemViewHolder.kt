...
class WebsiteItemViewHolder(binding: ItemWebsiteItemBinding) : AppBaseRecyclerViewHolder<ItemWebsiteItemBinding>(binding) {
  ...
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    ...
    ContextCompat.getColorStateList(activity!!,if (position % 2 != 0) R.color.white_smoke_1 else R.color.white)?.let { binding.mainContent.backgroundTintList=it }
    ...
    binding.mainContent.setOnClickListener { listener?.onItemClick(position, item, RecyclerViewActionType.WEBSITE_ITEM_CLICK.ordinal) }
  }
}
