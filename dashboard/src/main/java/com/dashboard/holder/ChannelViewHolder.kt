...
class ChannelViewHolder(binding: ItemChannelDBinding) :
  AppBaseRecyclerViewHolder<ItemChannelDBinding>(binding) {
  ...
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    ...
    val isConnect =
      ((data.isWhatsAppChannel() && data.channelActionData != null) || data.channelAccessToken != null || data.isGoogleSearch())
    getColor(if (isConnect) R.color.black_4f4f4f else R.color.warm_grey_two)?.let {
      binding.title.setTextColor(
        it
      )
    }
    (if (!isConnect) data.getDrawableInActiveNew(activity) else data.getDrawableActiveNew(activity))?.let {
    ...
    binding.mainContent.setOnClickListener {
      if (isConnect) listener?.onItemClick(
        position,
        item,
