...
object StaffNowFloatsRepository : AppBaseRepository<StaffNowFloatsRemoteData, AppBaseLocalService>() {
  ...
  fun updateGeneralService(request: UpdateRequestGeneralApt?): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.updateGeneralService(request), TaskCode.UPDATE_GENERAL_APT)
  }

  fun getGeneralService(fpId: String?, fpTag: String?): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.getGeneralService(fpId, fpTag), TaskCode.GET_GENERAL_APT)
  }
