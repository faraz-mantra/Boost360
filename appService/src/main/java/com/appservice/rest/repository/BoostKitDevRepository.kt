...
object BoostKitDevRepository : AppBaseRepository<BoostKitDevRemoteData, AppBaseLocalService>() {
  ...
  fun getTestimonialsList(testimonialType: String?, query: JSONObject?, skip: Int, limit: Int): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.getTestimonialsList(testimonialType, query, skip, limit), TaskCode.GET_TESTIMONIAL)
  }
...
//  }
  ...
  fun deleteTestimonials(testimonialType: String?, body: DeleteTestimonialRequest?): Observable<BaseResponse> {
    return makeRemoteRequest(remoteDataSource.deleteTestimonials(testimonialType, body), TaskCode.DELETE_TESTIMONIAL)
  }
