...
class BottomEcommerceTaxInvoices : BaseBottomSheetDialog<BottomSheetSetupTaxInvoicesForCustomerPurchaseBinding, BaseViewModel>() {
    ...
    fun setImage(parent: FragmentEcommerceCustomerInvoiceSetup) {
        ...
        binding?.layoutImagePreview?.ctvSize?.text = if (paymentProfileDetails?.taxDetails?.gSTDetails?.documentName.isNullOrEmpty()==true) paymentProfileDetails?.merchantSignature?.split("/")?.last()   else paymentProfileDetails?.taxDetails?.gSTDetails?.documentName
