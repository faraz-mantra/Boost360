...
class StaffListingViewHolder(binding: RecyclerItemStaffListingBinding) :
  AppBaseRecyclerViewHolder<RecyclerItemStaffListingBinding>(binding) {
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    val data = item as DataItem
    binding.ctvName.text = "${data.name}"
    binding.civImage.let {
      activity?.glideLoad(
        it,
        data.image.toString(),
    ...
    binding.btnViewProfile.setOnClickListener {
      listener?.onItemClick(
        position,
        data,
        RecyclerViewActionType.STAFF_LISTING_CLICK.ordinal
      )
    }

    if (item.isAvailable == false) {
      binding.linearRoot.setBackgroundColor(getApplicationContext()?.resources?.getColor(R.color.grey_f9f9f9)!!)
      binding.civImage.colorFilter =
        ColorMatrixColorFilter(ColorMatrix().apply { setSaturation(0f) })
      binding.btnViewProfile.background =
        ContextCompat.getDrawable(getApplicationContext()!!, R.drawable.rounded_stroke_grey_4)
      ...
      binding.btnViewProfile.background =
        ContextCompat.getDrawable(getApplicationContext()!!, R.drawable.rounded_stroke_orange_4)
