...
class ServiceListingViewHolder(binding: RecyclerItemServiceListingBinding) :
  AppBaseRecyclerViewHolder<RecyclerItemServiceListingBinding>(binding) {
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    ...
    binding.shareData.setOnClickListener {
      listener?.onItemClick(
        position,
        data,
        RecyclerViewActionType.SERVICE_DATA_SHARE_CLICK.ordinal
      )
    }
    binding.shareWhatsapp.setOnClickListener {
      listener?.onItemClick(
        position,
        data,
        RecyclerViewActionType.SERVICE_WHATS_APP_SHARE.ordinal
      )
    }
  }
