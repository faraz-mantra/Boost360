...
class ServiceListingViewHolder(binding: RecyclerItemServiceListingBinding) : AppBaseRecyclerViewHolder<RecyclerItemServiceListingBinding>(binding) {
  override fun bind(position: Int, item: BaseRecyclerViewItem) {
    ...
    if (data.price ?: 0.0 < data.discountedPrice ?: 0.0) {
      binding.labelBasePrice.gone()
    }
    if (0 >= data.discountedPrice ?: 0.0 || data.discountedPrice == data.price) {
      binding.labelBasePrice.gone()
    }
    binding.labelPrice.text = "${data.currency ?: "INR"} ${data.discountedPrice}"
    binding.labelPrice.text.apply {
      if (data.currency.isNullOrEmpty() || data.currency == "string" || data.currency.isBlank()) {
        "INR ${data.discountedPrice}"
      } else {
        "${data.currency ?: "INR"} ${data.discountedPrice}"
      }
    }
