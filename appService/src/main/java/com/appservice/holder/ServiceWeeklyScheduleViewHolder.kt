...
import com.appservice.databinding.RecyclerItemServiceSessionBinding
...
class ServiceWeeklyScheduleViewHolder(binding: RecyclerItemServiceSessionBinding) : AppBaseRecyclerViewHolder<RecyclerItemServiceSessionBinding>(binding) {
...
//if binding.layoutsessioncreate.visbility ==visible then add time slot will work else {
    ...
    fun addTimeSlots(data: ServiceModel) {
        if (data.timeSlots == null) {
            binding.llTimeSlot.removeAllViewsInLayout();
        } else {
            binding.llTimeSlot.addView(getTimeSlotView(data.timeSlots!!));
