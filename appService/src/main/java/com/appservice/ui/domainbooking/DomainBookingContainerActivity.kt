...
open class DomainBookingContainerActivity :
    ...
    private var searchDomainFragment: SearchDomainFragment? = null
    ...
    private fun setToolbarTitleNew(title: String, marginEnd: Int = resources.getDimensionPixelSize(R.dimen.size_44)) {
    ...
    override fun getNavigationIcon(): Drawable? {
        return when (type) {
            ...
            else -> ContextCompat.getDrawable(this, R.drawable.ic_domain_toolbar_back)
        }
    }
        ...
        return when (type) {
            ...
            FragmentType.ADDING_EXISTING_DOMAIN_FRAGMENT -> {
                addingExistingDomainFragment =
                    AddingExistingDomainFragment.newInstance(intent.extras)
                addingExistingDomainFragment
            }
            FragmentType.ACTIVE_DOMAIN_FRAGMENT -> {
                activeExistingDomainFragment =
                    ActiveExistingDomainFragment.newInstance(intent.extras)
                activeExistingDomainFragment
            }
            FragmentType.CONFIRMING_DOMAIN_FRAGMENT -> {
            ...
            FragmentType.SEARCH_DOMAIN_FRAGMENT -> {
                searchDomainFragment = SearchDomainFragment.newInstance(intent.extras)
                searchDomainFragment
            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        searchDomainFragment?.onActivityResult(requestCode, resultCode, data)
    }
}
