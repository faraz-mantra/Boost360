...
import com.appservice.model.panGst.PanGstResult
...
class ConfirmBusinessVerificationSheet : BaseBottomSheetDialog<SheetConfirmBusinessVerificationBinding, BaseViewModel>() {
  ...
  val imgUri by lazy { arguments?.getParcelable(IMG_URI) as Uri? }
  val pan by lazy { arguments?.getString(PAN) }
  val panName by lazy { arguments?.getString(PAN_NAME) }
  val gst by lazy { arguments?.getString(GST) }
  val rcm by lazy { arguments?.getBoolean(RCM) }
  val resultData by lazy { arguments?.getSerializable(RESULT_DATA) as? PanGstResult }
