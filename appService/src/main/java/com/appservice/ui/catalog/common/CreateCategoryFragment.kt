...
class CreateCategoryFragment : AppBaseFragment<FragmentCreateCategoryBinding, BaseViewModel>(),
  RecyclerItemClickListener {
  private var list: MutableList<Category> = mutableListOf(
    Category(name = "Books", 2),
  ...
  override fun onCreateView() {
    ...
    binding?.rvCategory?.apply {
      val adapterN =
        AppBaseRecyclerViewAdapter(baseActivity, ArrayList(list), this@CreateCategoryFragment)
      adapter = adapterN
    }
...
data class Category(
  ...
  var recyclerViewItem: Int = RecyclerViewItemType.CREATE_CATEGORY_ITEM_VIEW.getLayout()
