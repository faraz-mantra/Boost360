...
class ServiceDetailFragment : AppBaseFragment<FragmentServiceDetailBinding, ServiceViewModelV1>() {
    ...
    override fun onCreateView() {
        ...
        binding.serviceTimingSwitch.setOnToggledListener { _, isChecked ->
            ...
            if (isChecked) {
                ...
                binding.editWeekendScheduleView.visibility = View.GONE
                showServiceTimingsWindow()
            } else {
                binding.serviceTimingDescriptionView.visibility = View.GONE
    ...
    private fun setupUIColor() {
        changeColorOfSubstring(
            R.string.service_name_,
            R.color.black_4a4a4a,
            "*",
            binding?.tvServiceNameVw!!
        )
        changeColorOfSubstring(
            R.string.service_category_,
            R.color.black_4a4a4a,
            "*",
            binding?.tvServiceCategoryVw!!
        )
        changeColorOfSubstring(
            R.string.service_description_,
            R.color.black_4a4a4a,
            "*",
            binding?.tvServiceDescVw!!
        )
        changeColorOfSubstring(
            R.string.service_duration_minutes,
            R.color.black_4a4a4a,
            "*",
            binding?.tvServiceDurationVw!!
        )
    }

    private fun capLimitCheck() {
        val featureService =
            getCapData().filterFeature(CapLimitFeatureResponseItem.FeatureKey.PRODUCTCATALOGUE)
        val capLimitService = featureService?.filterProperty(PropertiesItem.KeyType.LIMIT)
        if (isEdit.not() && capLimitService != null) {
            viewModel?.getSearchListings(sessionLocal.fpTag, sessionLocal.fPID, "", 0, 5)
                ?.observeOnce(viewLifecycleOwner) {
                    ...
                    if (data?.count != null && capLimitService.getValueN() != null && data.count >= capLimitService.getValueN()!!) {
                        ...
                        showAlertCapLimit(
                            "Can't add the service catalogue, please activate your premium Add-ons plan.",
                            CapLimitFeatureResponseItem.FeatureKey.PRODUCTCATALOGUE.name
                        )
                    }
                }
        }
    ...
    private fun initProductFromBundle(data: Bundle?) {
        ...
        if (isEdit) getServiceDetailObject(p?.id) else {
            ...
            if (isDoctorClinic.not()) {
                showProgress()
                hitApi(
                    viewModel?.getAppointmentCatalogStatus(fpId ?: "", clientId ?: ""),
                    R.string.unable_to_fetch_default_gst_slab
                )
            }
        }
    }
        ...
        when (v) {
            ...
            binding?.btnOtherInfo -> {
                ...
                bundle.putSerializable(
                    IntentConstant.SERVICE_TIMING_DATA.name,
            ...
            binding?.editWeekendScheduleView -> {
                try {
                    val businessHoursIntent = Intent(
                        activity!!.applicationContext,
                        Class.forName("com.nowfloats.BusinessProfile.UI.UI.BusinessHoursActivity")
                    )
...
//    this.serviceTimingList?.map { it.isToggle = (it.day.isNullOrEmpty().not() && it.time?.from.isNullOrEmpty().not()) }
    ...
    private fun uploadSecondaryImages() {
        ...
        if (images.isNullOrEmpty().not()) {
            ...
            images.forEach { fileData ->
                val request =
                    UploadImageRequest.getInstance(1, product?.productId!!, fileData.getFile()!!)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ImagePicker.IMAGE_PICKER_REQUEST_CODE && resultCode == AppCompatActivity.RESULT_OK) {
            ...
            this.product =
                data?.getSerializableExtra(IntentConstant.PRODUCT_DATA.name) as? ServiceModelV1
            this.serviceTimingList =
        ...
        sessionLocal.fPID?.let { fpid ->
            ...
            viewModel?.getFpDetails(fpid, map)?.observeOnce(this) {
                ...
                if (!sessionLocal.noServiceSlot!!) {
                    ...
                    checkValidTimings(response?.timings)
    ...
    private fun checkValidTimings(timings: ArrayList<TimingDetailsModel>?) {
        val days = arrayOf("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")
        val aList = ArrayList<Int>()
        if (timings != null) {
            timings.add(timings[0])
            timings.removeAt(0)

            for (i in 0 until timings.size) {
                if (timings[i].from != "00" && timings[i].to != "00") {
                    aList.add(i)
                }
            }

            val dayTimings = arrayOf("NIL", "NIL", "NIL")

            when (aList.size) {
                0 -> {
                    dayTimings[0] = "  Mon : Closed"
                    dayTimings[1] = "  Tue : Closed"
                    dayTimings[2] = "  Wed : Closed"
                }
                1 -> {
                    aList.add((aList[0] + 1) % 7)
                    aList.add((aList[0] + 2) % 7)
                    dayTimings[0] =
                        "  " + days[aList[0]] + " : " + timings[aList[0]].from + " - " + timings[aList[0]].to
                    dayTimings[1] = "  " + days[aList[1]] + " : Closed"
                    dayTimings[2] = "  " + days[aList[2]] + " : Closed"
                }
                2 -> {
                    dayTimings[0] =
                        "  " + days[aList[0]] + " : " + timings[aList[0]].from + " - " + timings[aList[0]].to
                    dayTimings[1] =
                        "  " + days[aList[1]] + " : " + timings[aList[1]].from + " - " + timings[aList[1]].to
                    if (aList.contains((aList[1] + 1) % 7)) {
                        aList.add(((aList[1] + 1) % 7) + 1)
                    } else {
                        aList.add(((aList[1] + 1) % 7))
                    }
                    dayTimings[2] = "  " + days[aList[2]] + " : Closed"
                }
                else -> {
                    ...
                    dayTimings[1] =
                        "  " + days[aList[1]] + " : " + timings[aList[1]].from + " - " + timings[aList[1]].to
                    dayTimings[2] =
                        "  " + days[aList[2]] + " : " + timings[aList[2]].from + " - " + timings[aList[2]].to
                }
            }
            ...
            for (i in 0 until 3) {
                val timeView = CustomTextView(activity!!.applicationContext)
                timeView.setPadding(0, 8, 0, 8)
                timeView.setCompoundDrawablesWithIntrinsicBounds(
                    R.drawable.circle_bullets,
                    0,
                    0,
                    0
                );
                timeView.text = dayTimings[i]
                binding.timingsView.addView(timeView)
            }
        }
    }
    ...
    private fun showServiceTimingsWindow() {
        ...
        val compactDrawable = ContextCompat.getDrawable(
            activity!!.applicationContext,
            R.drawable.ic_arrow_top_right_yellow
        )
...
//                try {
//                    val businessProfileIntent = Intent(
//                        activity!!.applicationContext,
//                        Class.forName("com.dashboard.controller.DashboardFragmentContainerActivity")
//                    )
//                    businessProfileIntent.setFragmentType(FragmentType.FRAGMENT_BUSINESS_PROFILE)
...
//            }
//        }
//        spannableString.setSpan(clickableSpan, 81, 97, Spannable.SPAN_INCLUSIVE_EXCLUSIVE)
//        binding.serviceTimingDescriptionView.isClickable = true;
//        binding.serviceTimingDescriptionView.movementMethod = LinkMovementMethod.getInstance()
    }
}
