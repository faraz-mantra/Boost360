...
import com.appservice.databinding.FragmentRoomDetailsBinding
...
class RoomDetailFragment : AppBaseFragment<FragmentRoomDetailsBinding, ProductViewModel>() {
    ...
    override fun getLayout(): Int {
        return R.layout.fragment_room_details
    }
    ...
    override fun onClick(v: View) {
        ...
        when (v) {
            ...
            binding.btnOtherInfo -> {
                ...
                startFragmentActivity(FragmentType.CHECKKINN_ROOM_PRODUCT_INFORMATION, bundle, isResult = true, requestCode = RC_PRODCUT_INFO)
            }
        ...
        if (secondaryImageList.isNullOrEmpty().not()) {
            ...
            secondaryImageList.forEach { image ->
                ...
                viewModel?.addProductImage(request)?.observeOnce(viewLifecycleOwner) {
                    checkPosition += 1
                    if (it.isSuccess()) {
                        Log.d(RoomDetailFragment::class.java.name, "$it")
...
//    }
    ...
    private fun openImagePicker() {
        ...
        filterSheet.show(
            this@RoomDetailFragment.parentFragmentManager,
