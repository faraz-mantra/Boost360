...
data class ItemN(
  ...
  fun scheduledStartDate(): String {
    ...
    return if (scheduledDate.isNotEmpty()) {
      var startTime = Product?.extraItemProductConsultation()?.startTime()?.toUpperCase(Locale.ROOT)
      if (startTime?.contains("AM") == true || startTime?.contains("PM") == true) {
        startTime = parseDate(startTime, DateUtils.FORMAT_HH_MM_A, DateUtils.FORMAT_HH_MM)
      }
      return "${scheduledDate}T${startTime}:00.000Z"
    } else ""
  }
  ...
  fun scheduledEndDate(): String {
    val scheduledDate = getScheduledDate()
    return if (scheduledDate.isNotEmpty()) {
      var endTime = Product?.extraItemProductConsultation()?.endTime()?.toUpperCase(Locale.ROOT)
      if (endTime?.contains("AM") == true || endTime?.contains("PM") == true) {
        endTime = parseDate(endTime, DateUtils.FORMAT_HH_MM_A, DateUtils.FORMAT_HH_MM)
      }
      return "${scheduledDate}T${endTime}:00.000Z"
    } else ""
  }
  ...
  fun getScheduledDate(): String {
    ...
    return if (extraConsultation != null) {
      ...
      if (dateString.isNullOrEmpty()) dateString = parseDate(extraConsultation.scheduledDateTime(), DateUtils.FORMAT_YYYY_MM_DD, DateUtils.FORMAT_YYYY_MM_DD)
      dateString ?: ""
    } else ""
  }
  ...
  fun getScheduleDateAndTime(): String? {
    return try {
      ...
      var startTime = Product?.extraItemProductConsultation()?.startTime()?.toUpperCase(Locale.ROOT)
      var endTime = Product?.extraItemProductConsultation()?.endTime()?.toUpperCase(Locale.ROOT)
      if ((startTime?.contains("AM") == true || startTime?.contains("PM") == true).not()) {
        startTime = parseDate(startTime, DateUtils.FORMAT_HH_MM, DateUtils.FORMAT_HH_MM_A)
      }
      if ((endTime?.contains("AM") == true || startTime?.contains("PM") == true).not()) {
        endTime = parseDate(endTime, DateUtils.FORMAT_HH_MM, DateUtils.FORMAT_HH_MM_A)
      }
