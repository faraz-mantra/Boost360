...
class NewBookingFragmentOne : BaseInventoryFragment<FragmentNewBookingOneBinding>() {
  ...
  private fun setAdapterForAppointmentSchedule() {
    binding?.recyclerViewAppointmentSchedule?.post {
      val adapter = AppBaseRecyclerViewAdapter(baseActivity, AppointmentScheduleModel().getData())
  ...
  private fun clickChooseAppointTypeItem(item: AppointMentTypeModel?) {
    selectAppointmentList.forEach { it.isSelected = it.appointmentTypeSelectedName == item?.appointmentTypeSelectedName }
  }

  private fun clickChoosePurposeItem(item: ChoosePurposeModel?) {
    choosePurposeList.forEach { it.isSelected = (it.choosePurposeSelectedName == item?.choosePurposeSelectedName) }
  }
