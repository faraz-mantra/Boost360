...
//        binding?.radioVideoConsultation?.buttonTintList = colorStateList
        ...
        binding?.radioGroup?.setOnCheckedChangeListener { group, checkedId ->
            if(checkedId == binding?.radioInClinic?.id){
                ...
                binding?.radioVideoConsultation?.setTextColor(getColor(R.color.warm_grey_two))
                isVideoConsult = false
            }else if(checkedId == binding?.radioVideoConsultation?.id){
                binding?.radioVideoConsultation?.setTextColor(getColor(R.color.light_blue_one))
                binding?.radioInClinic?.setTextColor(getColor(R.color.warm_grey_two))
                isVideoConsult = true
            }
        }

        setOnClickListener(
                binding?.edtConsultingService, binding?.edtStartTime, binding?.btnCreate, binding?.edtGender
        )
