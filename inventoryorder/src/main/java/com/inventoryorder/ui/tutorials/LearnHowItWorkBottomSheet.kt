...
import com.inventoryorder.model.AUTHORIZATION_WEBSITE_EDU
...
class LearnHowItWorkBottomSheet : BaseBottomSheetDialog<BottomSheetLearnHowItWorksBinding, TutorialViewModel>() {
    ...
    private var dataTutorials: TutorialsDataResponse? = null
...
//        getStaffTutorial()
    ...
    private fun setViews(data: LearnAboutAppointmentMgmt?) {
    ...
    private fun setViews(data: TutorialsDataResponse?) {
        binding?.ctvHowItWorksContent?.text = fromHtml(data?.data?.get(0)?.heroSection?.description).toString()
        val pagerAdapter = PagerTutorialAdapter(childFragmentManager, behavior = FragmentStatePagerAdapter.BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT, data?.data?.get(0)?.videos)
        binding?.vpVideos?.adapter = pagerAdapter
        binding?.diPagerIndicator?.setViewPager(binding?.vpVideos!!)
        loadTips(data?.data?.get(0)?.tips)
        binding?.ctvVideosHeading?.text = "${if (data?.data?.get(0)?.videos?.size ?: 0 > 1) "VIDEOS" else "VIDEO"} (${data?.data?.get(0)?.videos?.size})"
    }

    private fun getTutorialData() {

        val userSessionManager = UserSessionManager(requireActivity())
        val fPID = userSessionManager.fPID
        viewModel?.getTutorialsData("6070004f026c4600019d7876", AUTHORIZATION_WEBSITE_EDU)?.observeOnce(
                viewLifecycleOwner, {
            if (it.isSuccess()) {
                this.dataTutorials = it as? TutorialsDataResponse
                setViews(dataTutorials)
    ...
    private fun loadTips(tips: List<String?>?) {
        tips?.forEach {
        ...
        when (v) {
            ...
            binding?.civSpeakTips -> {
                if (textToSpeechEngine.isSpeaking) {
                    ...
                    speak(data?.tips?.toString()?:dataTutorials?.data?.get(0)?.tips.toString())
                }
            ...
            binding?.actionReadFaq -> {
                ...
                val bundle = Bundle()
                bundle.putSerializable(IntentConstant.DATA_FAQS.name, dataTutorials?.data?.map { it?.faqs }?.toTypedArray())
                bottomSheetAppointmentFaq.arguments = bundle
...
class PagerTutorialAdapter(fm: FragmentManager, behavior: Int, private val videos: List<Video?>?) : FragmentStatePagerAdapter(fm, behavior) {
    override fun getCount(): Int {
        return videos?.size ?: 0
    }
...
class VideoFragment : AppBaseFragment<FragmentVideoPagerItemBinding, TutorialViewModel>() {
    companion object {
        ...
        fun newInstance(videoItem: Video): VideoFragment {
            val args = Bundle()
            args.putSerializable(IntentConstant.VIDEO_ITEM.name, videoItem)
            val fragment = VideoFragment()
            fragment.arguments = args
            return fragment
        }
    ...
    override fun onCreateView() {
        ...
        if (videosItem!=null){
            setView(videosItem.videoTitle,videosItem.videoLength,videosItem.videoThumbnails)
            binding?.root?.setOnClickListener { openTutorialBottomSheet(videosItem) }

        }else
        {
            val tutorialData = arguments?.getSerializable(IntentConstant.VIDEO_ITEM.name) as? Video
            setView(tutorialData?.title, tutorialData?.duration,tutorialData?.thumbnail?.url)
            binding?.root?.setOnClickListener { openTutorialBottomSheet(tutorialData) }
    ...
    private fun openTutorialBottomSheet(videosItem: Video?) {
        val tutorialVideosBottomSheet = TutorialVideosBottomSheet()
        val bundle = Bundle()
        bundle.putSerializable(IntentConstant.VIDEO_ITEM.name, videosItem)
        tutorialVideosBottomSheet.arguments = bundle
    ...
    private fun setView(videoTitle: String?,videoLength: String?, videoThumbnails: String?) {
        binding?.ctvVideoTitle?.text = videoTitle
        binding?.ctvVideoDuration?.text = videoLength
//    Glide.with(baseActivity).load(videosItem?.videoUrl).apply(RequestOptions()).thumbnail(0.1f).into(binding?.videoThumbnails!!)
        Glide.with(baseActivity).load(videoThumbnails).into(binding?.videoThumbnails!!)
    }
}
