...
class NewBookingFragmentTwo : BaseInventoryFragment<FragmentNewBookingTwoBinding>() {
  ...
  override fun onClick(v: View) {
    ...
    when (v) {
      ...
      binding?.buttonCreateBooking -> validateDetails(
        eTName.text.toString(),
        eTPhoneNumber.text.toString(),
        etEmail.text.toString(),
        tvSetGender.text.toString()
      )
      binding?.buttonPayAtClinic -> setBacUI(
        binding?.buttonPayAtClinic,
        binding?.buttonPayOnline,
        R.drawable.payment_bg_right
      )
      binding?.buttonPayOnline -> setBacUI(
        binding?.buttonPayOnline,
        binding?.buttonPayAtClinic,
        R.drawable.payment_bg_left
      )
  ...
  private fun showBottomSheetDialogSelectGender() {
    ...
    selectGenderBottomSheetDialog?.show(
      this.parentFragmentManager,
      SelectGenderBottomSheetDialog::class.java.name
    )
  }
  ...
  private fun validateDetails(
    ...
    if (TextUtils.isEmpty(customerEmail) || !Patterns.EMAIL_ADDRESS.matcher(customerEmail)
        .matches()
    ) {
    ...
    if (!TextUtils.isEmpty(customerName) && !TextUtils.isEmpty(customerPhoneNumber) && !TextUtils.isEmpty(
        customerEmail
      ) && !TextUtils.isEmpty(selectGender)
    ) {
