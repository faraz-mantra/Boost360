...
class DeliveryViewHolder(binding: ItemBottomSheetPickUpDeliveryOptionBinding) :
  AppBaseRecyclerViewHolder<ItemBottomSheetPickUpDeliveryOptionBinding>(binding) {
  ...
  private fun setDataForDeliveryOptions(model: DeliveryModel) {
    ...
    binding.mainView.setOnClickListener {
      listener?.onItemClick(
        adapterPosition,
        model,
        RecyclerViewActionType.DELIVERY_ITEM_CLICKED.ordinal
      )
    }
  }
