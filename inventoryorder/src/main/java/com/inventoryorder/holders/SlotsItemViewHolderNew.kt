...
class SlotsItemViewHolderNew(binding: ItemTimeSlotAppointmentNewBinding) :
  AppBaseRecyclerViewHolder<ItemTimeSlotAppointmentNewBinding>(binding) {
    ...
    binding?.linearRoot?.setOnClickListener {
      if (data?.IsAvailable == true) {
        listener?.onItemClick(
          adapterPosition,
          data,
          RecyclerViewActionType.TIME_SLOT_CLICKED.ordinal
        )
      }
    }
  }
