...
class VideoListViewHolder(binding: RecyclerItemListVideosBinding) :
  AppBaseRecyclerViewHolder<RecyclerItemListVideosBinding>(binding) {
...
//        Glide.with(BaseOrderApplication.instance.applicationContext).load(retrieveVideoFrameFromVideo(data?.videoUrl)).into(binding.videoThumbnails)
    ...
    binding.root.setOnClickListener {
      listener?.onItemClick(
        position,
        item,
        actionType = RecyclerViewActionType.VIDEO_ITEM_CLICK.ordinal
      )
    }
  }
