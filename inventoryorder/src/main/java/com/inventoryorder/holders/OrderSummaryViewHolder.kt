    ...
    override fun bind(position: Int, item: BaseRecyclerViewItem) {
        ...
        activity?.let {
            ...
            binding.mainView.background = ContextCompat.getDrawable(it,
                    takeIf { data.isSelected.not() }?.let { R.drawable.bg_rounded_30 } ?: (R.drawable.bg_rounded_solid_30)
            )

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                binding.count.backgroundTintList = ContextCompat.getColorStateList(it,
                        takeIf { data.isSelected.not() }?.let { data.color ?: R.color.colorAccent } ?: (R.color.white)
                )
            }
...
//      }
        ...
        binding.mainView.setOnClickListener {
            listener?.onItemClick(adapterPosition, data, RecyclerViewActionType.ORDER_SUMMARY_CLICKED.ordinal)
        }
    }
}