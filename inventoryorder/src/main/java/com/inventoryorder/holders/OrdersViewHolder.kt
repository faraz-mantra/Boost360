...
//    binding.itemCount.text = "$sizeItem ${takeIf { sizeItem > 1 }?.let { "Items" } ?: "Item"}"
    ...
    if (btnStatusMenu.isNullOrEmpty().not()) {
      ...
      binding.tvDropdownOrderStatus.setOnClickListener { listener?.onItemClick(adapterPosition, order, RecyclerViewActionType.ORDER_BUTTON_CLICKED.ordinal) }
    } else binding.lytStatusBtn.gone()
    ...
    if (btnStatusMenu.isNullOrEmpty()) {
      ...
      binding.ivDropdown.setOnClickListener { listener?.onItemClickView(adapterPosition, it, order, RecyclerViewActionType.BUTTON_ACTION_ITEM.ordinal) }
