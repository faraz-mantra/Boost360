...
object ApiWithFloatRepository : AppBaseRepository<WithFloatDataSource, AppBaseLocalService>() {
  ...
  fun getUserMessageCount(
    fpId: String?, clientId: String?, startDate: String?, endDate: String?
  ): Observable<BaseResponse> {
    return ApiWithFloatRepository.makeRemoteRequest(
      remoteDataSource.getUserMessageCount(fpId, clientId, startDate, endDate), TaskCode.GET_USER_MESSAGE_COUNT
    )
  }

  fun getSubscriberCount(fpTag: String?, clientId: String?, startDate: String?, endDate: String?): Observable<BaseResponse> {
    return ApiWithFloatRepository.makeRemoteRequest(
      remoteDataSource.getSubscriberCount(fpTag, clientId, startDate, endDate), TaskCode.GET_SUBSCRIBER_COUNT
    )
  }

  fun getUserCallSummary(
    clientId: String?, fpIdParent: String?, identifierType: String?, startDate: String?, endDate: String?
  ): Observable<BaseResponse> {
    return makeRemoteRequest(
      remoteDataSource.getUserCallSummary(clientId, fpIdParent, identifierType, startDate, endDate), TaskCode.GET_USER_CALL_SUMMARY
    )
  }

  fun getMapVisits(fpTag: String?, mapData: Map<String, String>?): Observable<BaseResponse> {
    return makeRemoteRequest(
      remoteDataSource.getMapVisits(fpTag, mapData), TaskCode.GET_MAP_SUMMARY
    )
  }
