...
object ApiWithFloatRepository : AppBaseRepository<WithFloatDataSource, AppBaseLocalService>() {
  ...
  fun getUserSummary(
    ...
    return ApiWithFloatRepository.makeRemoteRequest(
      remoteDataSource.getUserSummary(
        fpTag,
        clientId,
        fpIdParent,
        scope,
  ...
  fun getUserMessageCount(
    ...
    return ApiWithFloatRepository.makeRemoteRequest(
      remoteDataSource.getUserMessageCount(
        fpId,
        clientId,
        startDate,
        endDate
      ), TaskCode.GET_USER_MESSAGE_COUNT
    )
  }
  ...
  fun getSubscriberCount(
    ...
    return ApiWithFloatRepository.makeRemoteRequest(
      remoteDataSource.getSubscriberCount(
        fpTag,
        clientId,
        startDate,
        endDate
      ), TaskCode.GET_SUBSCRIBER_COUNT
    )
  }
  ...
  fun getUserCallSummary(
    ...
    return makeRemoteRequest(
      remoteDataSource.getUserCallSummary(
        clientId,
        fpIdParent,
        identifierType,
        startDate,
        endDate
      ), TaskCode.GET_USER_CALL_SUMMARY
    )
  }

  fun getMapVisits(fpTag: String?, mapData: Map<String, String>?): Observable<BaseResponse> {
    return makeRemoteRequest(
      remoteDataSource.getMapVisits(fpTag, mapData),
      TaskCode.GET_MAP_SUMMARY
    )
  }
