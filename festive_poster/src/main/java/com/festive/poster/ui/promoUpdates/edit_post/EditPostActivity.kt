...
import com.framework.webengageconstant.Promotional_Update_Edit_Loaded
import com.framework.webengageconstant.Promotional_Update_Preview_Post_Click
...
class EditPostActivity : AppBaseActivity<ActivityEditPostBinding, FestivePosterViewModel>() {
  ...
  override fun onCreateView() {
    WebEngageController.trackEvent(Promotional_Update_Edit_Loaded)
    initStt()
    sessionLocal = UserSessionManager(this)
    try {
      posterModel = convertStringToObj(intent.getStringExtra(IK_POSTER)!!)
      initUI()
      setOnClickListener(binding?.btnTapToEdit, binding?.captionLayout?.etInput, binding?.ivCloseEditing, binding?.tvPreviewAndPost, binding?.ivVoiceOver, binding?.ivCloseHashtag, binding?.alertBusinessLogo?.tvAddLogo)
    } catch (e: Exception) {
  ...
  private fun initUI() {
    binding?.captionLayout?.etInput?.setText(
      highlightHashTag(posterModel?.primaryText + "\n" + convertToHashTag(posterModel?.tags), R.color.black_4a4a4a, R.font.bold)
    )
    ...
    addHashTagFunction()
    binding?.tvHashtagSubtitle?.text = spanColor(getString(R.string.type_in_the_caption_to_create_your_own_hashtags), R.color.color395996, "#")
    ...
    binding?.captionLayout?.etInput?.setOnFocusChangeListener { _, hasFocus ->
      if (hasFocus) WebEngageController.trackEvent(Promotional_Update_Edit_Caption_Click)
    }
  }
  ...
  fun saveKeyValue() {
    viewModel.saveKeyValue(sessionLocal?.fPID, sessionLocal?.fpTag, arrayListOf(posterModel?.id!!), hashMapOf("test" to "test")).observe(this) {
      if (it.isSuccess()) {
      }
    }
  }
  ...
  /* fun replaceText(key:String,value:String){
       ...
       try {
           reader = BufferedReader(InputStreamReader(assets.open("frame_14.svg")));
