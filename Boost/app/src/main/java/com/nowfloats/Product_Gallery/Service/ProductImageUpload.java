 ...
 */
public class ProductImageUpload extends AsyncTask<String,String,String>{
    public String url="";
    ...
    public ProductImageUpload(String url,byte[] imageData,Product_Detail_Activity activity){
     ...
     try{
        if (flag){
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Methods.showSnackBarPositive(activity, "New product added");
                        }
                    });
                }
            }).start();
            ...
            if (activity.retryImage<3){
                new ProductImageUpload(url,imageData,activity).execute();
                activity.retryImage++;
            }else{
                activity.retryImage = 0;
                activity.runOnUiThread(new Runnable() {
