...
import com.boost.marketplace.utils.Utils
...
class MarketPlaceHomeViewModel() : BaseViewModel() {
  ...
  fun getAllFeaturesForMarketplace(application: Application, auth: String, fpid: String, clientId: String, expCode: String?, fpTag: String?, website_id: String?, activity: MarketPlaceActivity) {
...
//                                            marketplaceOffers.value = response.Data[0].marketplace_offers
  ...
  fun loadPackageUpdates(application: Application, website_id: String?, activity: MarketPlaceActivity) {
  ...
  fun getCartItemsBack(application: Application) {
    CompositeDisposable().add(
      AppDatabase.getInstance(application)!!
        .cartDao()
        .getCartItems()
  ...
  fun addItemToCart(application: Application, updatesModel: FeaturesModel, minMonth: Int) {
    updatesLoader.postValue(true)
...
//                            Toasty.error(this, "Something went wrong. Try Later..", Toast.LENGTH_LONG).show()
  ...
  fun addItemToCartPackage1(application: Application, cartItem: CartModel) {
    updatesLoader.postValue(false)
    Completable.fromAction {
      AppDatabase.getInstance(application)!!.cartDao()
        .insertToCart(cartItem)
  ...
  fun emptyCouponTable(application: Application) {
    Completable.fromAction {
      AppDatabase.getInstance(application)!!
        .couponsDao()
        .emptyCoupons()
