...
class MarketPlaceActivity : AppBaseActivity<ActivityMarketplaceBinding, MarketPlaceHomeViewModel>(), RecyclerItemClickListener {
  ...
  private var clientid: String = "2FA76D4AFCD84494BD609FDB4B3D76782F56AE790A3744198E6F517708CAAA21"
  private var website_id: String? = "5e7a3cf46e0572000109a5b2"


  private var experienceCode: String? = null
  private var fpName: String? = null
  ...
  private var isOpenCardFragment: Boolean = false
  private var isBackCart: Boolean = false
  private var isOpenHomeFragment: Boolean = false
  private var isOpenAddOnsFragment: Boolean = false
  private var userPurchasedWidgets = ArrayList<String>()
  ...
  private fun setData() {
    experienceCode = userSessionManager.fP_AppExperienceCode
    fpName = userSessionManager.fPName
    fpid = userSessionManager.fPID
    fpTag = userSessionManager.fpTag
    email = userSessionManager.fPEmail
    mobileNo = userSessionManager.userPrimaryMobile
    profileUrl = userSessionManager.fPLogo
  }
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    return when (item.itemId) {
      ...
      R.id.plan_history -> {
        startSpecificActivity(HistoryOrdersActivity::class.java)
  ...
  fun loadData() {
    fpid?.let { viewModel.getAllFeaturesForMarketplace(application, getAccessToken(), it,clientid,experienceCode,fpTag,website_id, activity = MarketPlaceActivity() ) }
  }
