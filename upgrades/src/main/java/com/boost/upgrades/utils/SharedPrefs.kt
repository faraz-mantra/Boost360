...
class SharedPrefs(activity: Activity) {
  ...
  fun getCartOrderInfo(): CreatePurchaseOrderResponse? {
    ...
    if (jsonString != null) {
      return Gson().fromJson<CreatePurchaseOrderResponse>(
        jsonString,
        object : TypeToken<CreatePurchaseOrderResponse>() {}.type
      )
    } else {
      return null
    }
  ...
  fun getCardIds(): List<String?>? {
    ...
    return if (TextUtils.isEmpty(str)) ArrayList() else Gson().fromJson(
      str,
