...
class RemoveAddonsFragment : BaseFragment(), RemoveItemsListener {
    ...
    override fun onCreateView(
        ...
        removeAddonsViewModelFactory = RemoveAddonsViewModelFactory(requireNotNull(requireActivity().application))

        viewModel = ViewModelProviders.of(requireActivity(), removeAddonsViewModelFactory).get(RemoveAddonsViewModel::class.java)
...
//                localStorage.setCartItem(cart_list!!)
                ...
                (activity as UpgradeActivity).replaceFragment(RemoveAddonConfirmationFragment.newInstance(), REMOVE_ADDONS_CONFIRMATION_FRAGMENT)
            }
        }
