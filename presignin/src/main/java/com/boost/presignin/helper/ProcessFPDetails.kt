...
class ProcessFPDetails(var session: UserSessionManager) {
  ...
  fun storeFPDetails(get_fp_details_model: UserFpDetailsResponse) {
    try {
      ...
      if (get_fp_details_model.accountManagerId != null && get_fp_details_model.accountManagerId != "null") session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_ACCOUNTMANAGERID, get_fp_details_model.accountManagerId)

      try {
        if (get_fp_details_model.category != null && get_fp_details_model.category.size > 0) {
          session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_CATEGORY, get_fp_details_model.category[0].key)
      ...
      session.storeFPDetails(Key_Preferences.GET_FP_WEBTEMPLATE_ID, get_fp_details_model.webTemplateId)
      ...
      session.storeFPDetails(Key_Preferences.PRODUCT_CATEGORY, get_fp_details_model.productCategoryVerb)
    ...
    try {
      if (get_fp_details_model.timings != null) {
        ...
        for (i in 0 until get_fp_details_model.timings.size) {
          when (i) {
            0 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_SUNDAY_START_TIME, get_fp_details_model.timings[0].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_SUNDAY_END_TIME, get_fp_details_model.timings[0].to)
              session.businessHours = true
            }
            1 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_MONDAY_START_TIME, get_fp_details_model.timings[1].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_MONDAY_END_TIME, get_fp_details_model.timings[1].to)
            }
            2 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_TUESDAY_START_TIME, get_fp_details_model.timings[2].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_TUESDAY_END_TIME, get_fp_details_model.timings[2].to)
            }
            3 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_WEDNESDAY_START_TIME, get_fp_details_model.timings[3].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_WEDNESDAY_END_TIME, get_fp_details_model.timings[3].to)
            }
            4 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_THURSDAY_START_TIME, get_fp_details_model.timings[4].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_THURSDAY_END_TIME, get_fp_details_model.timings[4].to)
            }
            5 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_FRIDAY_START_TIME, get_fp_details_model.timings[5].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_FRIDAY_END_TIME, get_fp_details_model.timings[5].to)
            }
            6 -> {
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_SATURDAY_START_TIME, get_fp_details_model.timings[6].from)
              session.storeFPDetails(Key_Preferences.GET_FP_DETAILS_SATURDAY_END_TIME, get_fp_details_model.timings[6].to)
            }
          }
        }
      }
