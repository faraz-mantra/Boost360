...
import io.separ.neural.inputmethod.indic.AssetFileAddress;
...
public class DictionaryInfoUtils {
     ...
     */
    public static int getMainDictionaryResourceIdIfAvailableForLocale(Context context, final Resources res,
                                                                      final Locale locale) {
        int resId;
        ...
        resId = res.getIdentifier(dictLanguage, "raw", RESOURCE_PACKAGE_NAME);
        if (resId != 0) {
            return resId;
        }
     ...
     * @return main dictionary resource id
     */
    public static int getMainDictionaryResourceId(Context context, final Resources res, final Locale locale) {
    ...
    private static DictionaryInfo createDictionaryInfoFromFileAddress(
            Context context, final AssetFileAddress fileAddress) {
        ...
        return new DictionaryInfo(context, id, locale, description, fileAddress, Integer.parseInt(version));
    }
            ...
            for (final File directory : directoryList) {
                ...
                for (final File dict : dicts) {
                    ...
                    final DictionaryInfo dictionaryInfo =
                            createDictionaryInfoFromFileAddress(context, fileAddress);
        ...
        for (final String localeString : assets.getLocales()) {
            ...
            final int resourceId =
                    DictionaryInfoUtils.getMainDictionaryResourceIdIfAvailableForLocale(
                            context, context.getResources(), locale);
            if (0 == resourceId) continue;
            final AssetFileAddress fileAddress =
                    BinaryDictionaryGetter.loadFallbackResource(context, resourceId);
            final DictionaryInfo dictionaryInfo = createDictionaryInfoFromFileAddress(context, fileAddress);
            // Protect against cases of a less-specific dictionary being found, like an
            // en dictionary being used for an en_US locale. In this case, the en dictionary
            // should be used for en_US but discounted for listing purposes.
