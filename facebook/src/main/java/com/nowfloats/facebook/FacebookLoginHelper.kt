...
interface FacebookLoginHelper {
  ...
  fun loginWithFacebook(
    fragment: Fragment,
    permissions: List<FacebookPermissions>,
    arePublishPermissions: Boolean = false
  ) {
...
//    else LoginManager.getInstance().logInWithReadPermissions(fragment, permissions.map { it.name })
  ...
  fun loginWithFacebook(
    ...
    if (arePublishPermissions) LoginManager.getInstance()
      .logInWithPublishPermissions(activity, permissions.map { it.name })
